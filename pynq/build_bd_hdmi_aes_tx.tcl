...
# EDID: use the dvi2rgb module reference's own stock 720p60 default file.
# A generated native 720p30 EDID was tried and reverted. The real HDMI
# source (this GPU) has no 720p30 output mode at all: its own "List All
# Modes" dialog lists only 50/59/60/75 Hz at 1280x720, never 30 Hz. 30 fps
# is produced instead by 2:1 frame decimation in HDMI_Axis_Packetizer.sv,
# against the real 60 Hz source. The vendor EDID file is never modified;
# this project simply does not overwrite the module reference's own copy
# of it, so Vivado populates it from the IP's own packaged default.
...
# 720p_edid.data is left as the module reference's own packaged 720p60
# default. It is never overwritten (see the EDID note near the top of this
# script).
...
# 720p_edid.data is regenerated by the step above from the IP's own
# packaged 720p60 default. It is not overwritten here.
...
puts "    - 720p60 HDMI input, 2:1 packetizer decimation to 30 fps wire rate"
