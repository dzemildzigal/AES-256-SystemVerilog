`timescale 1ns / 1ps
//////////////////////////////////////////////////////////////////////////////////

module DecryptPipelinedTestBench;
    reg clk;
    reg rst;
    reg start_i;
    reg [0:127] in;
    reg [0:1919] expanded_key;

    wire [0:127] out;
    wire valid_data;

    DecryptPipelined decrypt(
        .clk(clk),
        .rst(rst),
        .start_i(start_i),
        .in(in),
        .expanded_key(expanded_key),
        .out(out),
        .valid_data(valid_data)
    );

    integer errors;

    initial begin
        $display("[Decrypt Pipelined TB] Starting...");
        errors = 0;
        clk = 0;
        rst = 0;
        start_i = 0;
        in = '0;
        expanded_key = '0;

        // ---- Reset ----
        @(posedge clk);
        rst = 1;
        repeat(2) @(posedge clk);
        rst = 0;
        @(posedge clk);

        // ================================================================
        // Test 1: all-zero key, ct -> pt=0
        // ================================================================
        $display("  T1: all-zero key, decrypt dc95c078...");
        expanded_key = 1920';
        in = 128'hdc95c078a2408989ad48a21492842087;
        start_i = 1;
        @(posedge clk);
        #1; start_i = 0;
        repeat(14) @(posedge clk);
        #1;
        if (valid_data !== 1'b1) begin $display("FAIL T1: valid_data not asserted"); errors = errors + 1; end
        if (out !== 128'h00000000000000000000000000000000) begin $display("FAIL T1: wrong pt, got %h", out); errors = errors + 1; end
        @(posedge clk);
        #1;
        if (valid_data !== 1'b0) begin $display("FAIL T1: valid_data should be 0 after"); errors = errors + 1; end

        // ================================================================
        // Test 2: NIST key, ct -> pt=0
        // ================================================================
        $display("  T2: NIST key, decrypt f29000b6...");
        expanded_key = 1920';
        in = 128'hf29000b62a499fd0a9f39a6add2e7780;
        start_i = 1;
        @(posedge clk);
        #1; start_i = 0;
        repeat(14) @(posedge clk);
        #1;
        if (valid_data !== 1'b1) begin $display("FAIL T2: valid_data not asserted"); errors = errors + 1; end
        if (out !== 128'h00000000000000000000000000000000) begin $display("FAIL T2: wrong pt, got %h", out); errors = errors + 1; end

        // ================================================================
        // Test 3: NIST key, ct -> pt=00112233..eeff
        // ================================================================
        $display("  T3: NIST key, decrypt 8ea2b7ca...");
        in = 128'h8ea2b7ca516745bfeafc49904b496089;
        start_i = 1;
        @(posedge clk);
        #1; start_i = 0;
        repeat(14) @(posedge clk);
        #1;
        if (valid_data !== 1'b1) begin $display("FAIL T3: valid_data not asserted"); errors = errors + 1; end
        if (out !== 128'h00112233445566778899aabbccddeeff) begin $display("FAIL T3: wrong pt, got %h", out); errors = errors + 1; end

        // ================================================================
        // Test 4: custom key, ct -> "Hello world!  :)"
        // ================================================================
        $display("  T4: custom key, decrypt ea1044c3...");
        expanded_key = 1920';
        in = 128'hea1044c3b1139b6cf71095be1071b90d;
        start_i = 1;
        @(posedge clk);
        #1; start_i = 0;
        repeat(14) @(posedge clk);
        #1;
        if (valid_data !== 1'b1) begin $display("FAIL T4: valid_data not asserted"); errors = errors + 1; end
        if (out !== 128'h48656c6c6f20776f726c642120203a29) begin $display("FAIL T4: wrong pt, got %h", out); errors = errors + 1; end

        // ================================================================
        // Test 5: Continuous streaming — 4 blocks same NIST key
        //   Feed 4 ciphertexts back-to-back; expect 4 plaintexts in order
        // ================================================================
        $display("  T5: continuous 4-block decrypt stream, NIST key");
        expanded_key = 1920';
        start_i = 1;
        in = 128'hf29000b62a499fd0a9f39a6add2e7780;
        @(posedge clk);
        in = 128'h8ea2b7ca516745bfeafc49904b496089;
        @(posedge clk);
        in = 128'hf29000b62a499fd0a9f39a6add2e7780;
        @(posedge clk);
        in = 128'h8ea2b7ca516745bfeafc49904b496089;
        @(posedge clk);
        #1; start_i = 0;
        in = '0;
        repeat(10) @(posedge clk);
        @(posedge clk);
        #1;
        if (valid_data !== 1'b1) begin $display("FAIL T5a: valid_data not asserted"); errors = errors + 1; end
        if (out !== 128'h00000000000000000000000000000000) begin $display("FAIL T5a: wrong pt, got %h", out); errors = errors + 1; end
        @(posedge clk);
        #1;
        if (valid_data !== 1'b1) begin $display("FAIL T5b: valid_data not asserted"); errors = errors + 1; end
        if (out !== 128'h00112233445566778899aabbccddeeff) begin $display("FAIL T5b: wrong pt, got %h", out); errors = errors + 1; end
        @(posedge clk);
        #1;
        if (valid_data !== 1'b1) begin $display("FAIL T5c: valid_data not asserted"); errors = errors + 1; end
        if (out !== 128'h00000000000000000000000000000000) begin $display("FAIL T5c: wrong pt, got %h", out); errors = errors + 1; end
        @(posedge clk);
        #1;
        if (valid_data !== 1'b1) begin $display("FAIL T5d: valid_data not asserted"); errors = errors + 1; end
        if (out !== 128'h00112233445566778899aabbccddeeff) begin $display("FAIL T5d: wrong pt, got %h", out); errors = errors + 1; end
        @(posedge clk);
        #1;
        if (valid_data !== 1'b0) begin $display("FAIL T5: valid still high after stream"); errors = errors + 1; end

        // ================================================================
        // Summary
        // ================================================================
        if (errors == 0)
            $display("[Decrypt Pipelined TB] PASSED");
        else
            $display("[Decrypt Pipelined TB] FAILED with %0d errors", errors);
        $finish;
    end

    always #1 clk = ~clk;
endmodule
