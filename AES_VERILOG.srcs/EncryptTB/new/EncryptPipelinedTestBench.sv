`timescale 1ns / 1ps
//////////////////////////////////////////////////////////////////////////////////

module EncryptPipelinedTestBench;
    reg clk;
    reg rst;
    reg start_i;
    reg [0:127] in;
    reg [0:1919] expanded_key;

    wire [0:127] out;
    wire valid_data;

    EncryptPipelined encrypt(
        .clk(clk),
        .rst(rst),
        .start_i(start_i),
        .in(in),
        .expanded_key(expanded_key),
        .out(out),
        .valid_data(valid_data)
    );

    integer errors;

    initial begin
        $display("[Encrypt Pipelined TB] Starting...");
        errors = 0;
        clk = 0;
        rst = 0;
        start_i = 0;
        in = '0;
        expanded_key = '0;

        // ---- Reset ----
        @(posedge clk);
        rst = 1;
        repeat(2) @(posedge clk);
        rst = 0;
        @(posedge clk);

        // ================================================================
        // Test 1: Single block — all-zero key, all-zero plaintext
        // ================================================================
        $display("  T1: all-zero key, zero plaintext");
        expanded_key = 1920';
        in = 128'h00000000000000000000000000000000;
        start_i = 1;
        @(posedge clk);
        start_i = 0;
        repeat(14) @(posedge clk);
        #1;
        if (valid_data !== 1'b1) begin $display("FAIL T1: valid_data not asserted"); errors = errors + 1; end
        if (out !== 128'hdc95c078a2408989ad48a21492842087) begin $display("FAIL T1: wrong ct, got %h", out); errors = errors + 1; end
        @(posedge clk);
        #1;
        if (valid_data !== 1'b0) begin $display("FAIL T1: valid_data should be 0 after"); errors = errors + 1; end

        // ================================================================
        // Test 2: Single block — NIST key (000102..1f), zero plaintext
        // ================================================================
        $display("  T2: NIST key, zero plaintext");
        expanded_key = 1920';
        in = 128'h00000000000000000000000000000000;
        start_i = 1;
        @(posedge clk);
        start_i = 0;
        repeat(14) @(posedge clk);
        #1;
        if (valid_data !== 1'b1) begin $display("FAIL T2: valid_data not asserted"); errors = errors + 1; end
        if (out !== 128'hf29000b62a499fd0a9f39a6add2e7780) begin $display("FAIL T2: wrong ct, got %h", out); errors = errors + 1; end

        // ================================================================
        // Test 3: Single block — NIST key, NIST plaintext (00112233..eeff)
        // ================================================================
        $display("  T3: NIST key, NIST plaintext");
        in = 128'h00112233445566778899aabbccddeeff;
        start_i = 1;
        @(posedge clk);
        start_i = 0;
        repeat(14) @(posedge clk);
        #1;
        if (valid_data !== 1'b1) begin $display("FAIL T3: valid_data not asserted"); errors = errors + 1; end
        if (out !== 128'h8ea2b7ca516745bfeafc49904b496089) begin $display("FAIL T3: wrong ct, got %h", out); errors = errors + 1; end

        // ================================================================
        // Test 4: Single block — custom key, "Hello world!  :)"
        // ================================================================
        $display("  T4: custom key, Hello world");
        expanded_key = 1920';
        in = "Hello world!  :)";
        start_i = 1;
        @(posedge clk);
        start_i = 0;
        repeat(14) @(posedge clk);
        #1;
        if (valid_data !== 1'b1) begin $display("FAIL T4: valid_data not asserted"); errors = errors + 1; end
        if (out !== 128'h8e00ecc38998b9806a9559f59054aaa6) begin $display("FAIL T4: wrong ct, got %h", out); errors = errors + 1; end

        // ================================================================
        // Test 5: Continuous streaming — 4 blocks same NIST key
        //   Verifies pipeline throughput: 1 result/cycle after fill
        // ================================================================
        $display("  T5: continuous 4-block stream, NIST key");
        expanded_key = 1920';
        start_i = 1;
        in = 128'h00000000000000000000000000000000;
        @(posedge clk);
        in = 128'h00112233445566778899aabbccddeeff;
        @(posedge clk);
        in = 128'h00000000000000000000000000000000;
        @(posedge clk);
        in = 128'h00112233445566778899aabbccddeeff;
        @(posedge clk);
        start_i = 0;
        in = '0;
        repeat(10) @(posedge clk);
        // 4 feeds (cycles 0-3) + 10 idle + 1 more = 15 posedges for first result
        @(posedge clk);
        #1;
        if (valid_data !== 1'b1) begin $display("FAIL T5a: valid_data not asserted"); errors = errors + 1; end
        if (out !== 128'hf29000b62a499fd0a9f39a6add2e7780) begin $display("FAIL T5a: wrong ct, got %h", out); errors = errors + 1; end
        @(posedge clk);
        #1;
        if (valid_data !== 1'b1) begin $display("FAIL T5b: valid_data not asserted"); errors = errors + 1; end
        if (out !== 128'h8ea2b7ca516745bfeafc49904b496089) begin $display("FAIL T5b: wrong ct, got %h", out); errors = errors + 1; end
        @(posedge clk);
        #1;
        if (valid_data !== 1'b1) begin $display("FAIL T5c: valid_data not asserted"); errors = errors + 1; end
        if (out !== 128'hf29000b62a499fd0a9f39a6add2e7780) begin $display("FAIL T5c: wrong ct, got %h", out); errors = errors + 1; end
        @(posedge clk);
        #1;
        if (valid_data !== 1'b1) begin $display("FAIL T5d: valid_data not asserted"); errors = errors + 1; end
        if (out !== 128'h8ea2b7ca516745bfeafc49904b496089) begin $display("FAIL T5d: wrong ct, got %h", out); errors = errors + 1; end
        @(posedge clk);
        #1;
        if (valid_data !== 1'b0) begin $display("FAIL T5: valid still high after stream"); errors = errors + 1; end

        // ================================================================
        // Summary
        // ================================================================
        if (errors == 0)
            $display("[Encrypt Pipelined TB] PASSED");
        else
            $display("[Encrypt Pipelined TB] FAILED with %0d errors", errors);
        $finish;
    end

    always #1 clk = ~clk;
endmodule
